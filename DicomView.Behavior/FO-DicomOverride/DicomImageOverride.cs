...
namespace DicomView.Behaviors.Fo_DicomOverride
{

    /// <summary>
    /// DICOM Image class for image rendering.
    /// </summary>
    public class DicomImageOverride
    {
        #region FIELDS
        ...
        private readonly ConcurrentDictionary<int, IPixelDataOverride> _pixelsCache = new ConcurrentDictionary<int, IPixelDataOverride>();
        ...
        public DicomImageOverride(DicomDataset dataset, int frame = 0)
        {
            ShowOverlays = true;
        ...
        /// <param name="frame">Zero-based frame index.</param>
        public DicomImageOverride(string fileName, int frame = 0)
        ...
        public unsafe virtual void RenderImageOverride()
        {
            if (_renderedImageCache.TryGetValue(CurrentFrame, out var image))
            {
                var pixels = GetOrCreateCachedPixelData(CurrentFrame).RescaleOverride(_scale);

                var pipeline = GetOrCreateCachedFramePipeline(CurrentFrame);

                var graphic = new ImageGraphicOverride(pixels);
                ...
                if (ShowOverlays)
                {
                    foreach (var overlay in _overlays.Value)
                    {
                        ...
                        if (CurrentFrame + 1 < overlay.OriginFrame
                            || CurrentFrame + 1 > overlay.OriginFrame + overlay.NumberOfFrames - 1)
                        {
                            continue;
                        }

                        var og = new OverlayGraphicOverride(
                            PixelDataFactoryOverride.Create(overlay),
                            overlay.OriginX - 1,
                            overlay.OriginY - 1,
                            OverlayColor);
                        graphic.AddOverlay(og);
                ...
                var writeable = image.AsWriteableBitmap();
                writeable.Lock();
                try
                {
                    graphic.RenderImageOverride((int*)writeable.BackBuffer, pipeline.LUT);
        ...
        private IPixelDataOverride GetOrCreateCachedPixelData(int frame)
        {
        ...
        private IPixelDataOverride GetFrameData(int frame)
        {
            if (_dataset.InternalTransferSyntax.IsEncapsulated)
            {
                // decompress single frame from source dataset
                var transcoder = new DicomTranscoderOverride(
                    _dataset.InternalTransferSyntax,
                    DicomTransferSyntax.ExplicitVRLittleEndian);
                var pixels = transcoder.DecodePixelDataOverride(_dataset, frame);

                return pixels;
            }
            else
            {
                return PixelDataFactoryOverride.Create(_pixelDataCache, frame);
            }
        }
        ...
        private static DicomPixelData CreateDicomPixelData(DicomDataset dataset)
        {
            ...
            var internalTransferSyntax = typeof(DicomDataset).GetProperty(nameof(DicomDataset.InternalTransferSyntax));
            internalTransferSyntax?.SetValue(clone, DicomTransferSyntax.ExplicitVRLittleEndian);
